...
import { SERVES_IN } from "../../lib/types";
import { Button } from "../ui/Button";
import { Select } from "../ui/Select";

interface UnitPersonnelProps {
  unitId: string;
}

/** serves_in picker/list for characters (FR4.1/FR4.4) -- mirrors
 * SpeciesMembers.tsx's picker shape directly. */
export function UnitPersonnel({ unitId }: UnitPersonnelProps) {
  ...
  const personnelLinks = relationships.filter(
    (r) => r.relationship_type === SERVES_IN && r.target_entity_id === unitId,
  );
  const linkedCharacterIds = new Set(personnelLinks.map((r) => r.source_entity_id));
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {personnelLinks.length === 0 ? (
        <p className="text-xs text-[var(--color-text-tertiary)]">
          No characters serve in this unit yet.
        </p>
      ) : (
        <ul className="flex flex-col gap-1.5">
          {personnelLinks.map((rel) => (
            <li
              key={rel.id}
