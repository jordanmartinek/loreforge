...
import { MEMBER_OF } from "../../lib/types";
import { Button } from "../ui/Button";
import { Select } from "../ui/Select";

interface SpeciesMembersProps {
  speciesId: string;
}

/** member_of picker/list for characters (FR4.1/FR4.3) -- mirrors
 * LocationRelations.tsx's located_at picker shape, narrowed to one source
 * entity type (character) instead of two (character/event). */
export function SpeciesMembers({ speciesId }: SpeciesMembersProps) {
  ...
  const memberLinks = relationships.filter(
    (r) => r.relationship_type === MEMBER_OF && r.target_entity_id === speciesId,
  );
    ...
    <div className="flex flex-col gap-3">
      ...
      {memberLinks.length === 0 ? (
        <p className="text-xs text-[var(--color-text-tertiary)]">
          No characters belong to this species yet.
        </p>
      ) : (
